...
import { expand, map, reduce, shareReplay, concat } from 'rxjs/operators';
...
export class CharacterService {
  ...
  private getCharacters(): Observable<Character[]> {
    return combineLatest([
      this.http.get<CharactersResponse>(`${this.apiUrl}/people`).pipe(
        expand(resp => resp.next ? this.http.get<CharactersResponse>(resp.next) : EMPTY),
        reduce((acc: any, current) => acc.concat(current.results), []),
